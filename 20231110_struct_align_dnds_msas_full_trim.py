...
#
#Output: 
#  Strict trimming: msas/structural/tm_align/trim_complete/
##  threaded original alignment: msas/structural/tm_align/cds_aln/  ## Previously calculated
#  Trimmed CDS alignment: msas/structural/tm_align/cds_trim_complete/
##  Renaming Map:  msas/structural/tm_align/seq_name_map/  ## Previously calculated
#
#  The final output are phylip files with short names strictly trimmed and formatted for use with Code ML.
#
#  msas/structural/tm_align/cds_trim_complete/<alignment>.tm.fasta.clipkit.cds.renamed.codeML.phy'
...
selected_alignments = ['OG4316_REF_Scer_AF-P00424-F1-model_v2',
...
#['OG2645_REF_Scer_AF-P05375-F1-model_v2']#['OG4150_REF_Scer_AF-P07256-F1-model_v2', 'OG2603_REF_Scer_AF-P50076-F1-model_v2', 'OG2845_REF_Scer_AF-P43577-F1-model_v2', 'OG3677_REF_Scer_AF-P47125-F1-model_v2', 'OG1299_REF_Scer_AF-P00549-F1-model_v2']
...
for alignment in selected_alignments: 
    ...
    og_pep_msa_fname = aln_dir + os.sep + os.path.normpath('fasta_renamed/' + alignment + '.tm.fasta')
    
    #Complete Trimming for codon alignments    
    print('Complete trimming')
    #clipkit_cmd = ['clipkit', og_pep_msa_fname, '-m','gappy' ,'-g','0.1', '-l']
    clipkit_cmd = ['clipkit', og_pep_msa_fname, '-m','gappy' ,'-g','0', '-l']
    ...
    for suffix in suffixes: 
        ...
        shutil.move(og_pep_msa_fname + suffix, 
                    aln_dir + os.sep + os.path.normpath('trim_complete/' + alignment + '.tm.fasta' + suffix)
                   )
    ...
    msa_pep_trimmed = aln_dir + os.sep + os.path.normpath('trim_complete/' + alignment + '.tm.fasta.clipkit')
    ...
    if (trimmed_msa_len/med_length)>trim_msa_thresh:    
        ...
        msa_cds_trimmed = aln_dir + os.sep + os.path.normpath('cds_trim_complete/' + alignment +  '.tm.fasta.clipkit.cds')
...
filtered_og_log_fname = aln_dir + os.sep + os.path.normpath('cds_trim_complete/filtered_og.log')

with open(filtered_og_log_fname, 'w') as f_out: 
    f_out.write('Orthogroups filtered out when running 20231110_struct_align_dnds_msas_full_trim.py because strict trimming of alignment was below trim_msa_thresh=' + str(trim_msa_thresh) + ' * average sequence length threshold\n')
